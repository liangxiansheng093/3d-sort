...
cv::Mat TrackerState::toMat(void) {
    ...
    mat.at<float>(5, 0) = velocity(0);
    mat.at<float>(6, 0) = velocity(1);
    mat.at<float>(7, 0) = velocity(2);
    mat.at<float>(8, 0) = vs;
    mat.at<float>(9, 0) = vr;
    mat.at<float>(10, 0) = acceleration(0);
    mat.at<float>(11, 0) = acceleration(1);
    mat.at<float>(12, 0) = acceleration(2);
